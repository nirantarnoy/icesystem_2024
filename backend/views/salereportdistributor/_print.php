...
function getOrderQty2($customer_id, $product_id, $from_date, $to_date)
{
    ...
    if ($customer_id && $product_id) {
        $model_qty = \common\models\TransactionDistributorSale::find()->select(['SUM(credit_qty) as credit_qty', 'SUM(cash_qty) as cash_qty'])->where(['BETWEEN', 'date(trans_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d',strtotime($to_date))])->andFilterWhere(['customer_id' => $customer_id, 'product_id' => $product_id])->groupBy(['product_id'])->one();
        if ($model_qty != null) {
