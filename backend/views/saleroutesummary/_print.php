                ...
                <tr>
                    ...
                    <td style="width: 20%">
                        ...
                        echo \kartik\select2\Select2::widget([
                            'name' => 'find_route',
                            'data' => \yii\helpers\ArrayHelper::map(\backend\models\Deliveryroute::find()->where(['status' => 1, 'is_other_branch' => 0,'branch_id'=>1])->all(), 'id', 'name'),
                            'value'=> $find_route,
                            'options' => [
...
function getOrderQty2($customer_id, $product_id, $from_date, $to_date)
{
    ...
    if ($customer_id && $product_id) {
        $model_qty = \common\models\QuerySaleMobileDataNew::find()->select(['SUM(line_qty_credit) as line_qty_credit', 'SUM(line_qty_cash) as line_qty_cash'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])->andFilterWhere(['customer_id' => $customer_id, 'product_id' => $product_id])->groupBy(['product_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->line_qty_credit + $model_qty->line_qty_cash);
        }
    }
    return $data;
}
