...
class Deviceregister extends \common\models\DeviceRegister
{
...
//            ],
    ...
    public function getRouteName($id){
        $html = '';
        $model = \common\models\DeliveryRouteDevice::find()->where(['device_register_id'=>$id])->all();
        if($model){
            foreach ($model as $value) {
                $name = \backend\models\Deliveryroute::find()->where(['id'=>$value->delivery_route_id])->one()->name;
                if($name){
                    $html.='<div class="btn btn-sm btn-secondary">'.$name.'</div><span> </span>';
                }
            }
