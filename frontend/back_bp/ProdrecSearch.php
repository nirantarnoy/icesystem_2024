...
{
    public $from_date,$to_date,$prodrec_type,$production_view_type_id;
    public function rules()
    {
        return [
            [['id', 'company_id', 'branch_id', 'product_id', 'warehouse_id', 'location_id', 'qty', 'created_at'], 'integer'],
            [['journal_no', 'trans_date', 'lot_no','activity_type_id', 'created_by','from_date','to_date','prodrec_type','production_view_type_id'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere([
            ...
            'qty' => $this->qty,
            // 'production_type' => $this->production_view_type_id,
        ...
        if($this->prodrec_type!=''){
            if($this->prodrec_type != 'RTF'){
                $query->andFilterWhere(['LIKE','journal_no',$this->prodrec_type])
                    ->andFilterWhere(['=','production_type',$this->production_view_type_id]);
            }else{
                $query->andFilterWhere(['LIKE','journal_no',$this->prodrec_type])
                    //   ->andFilterWhere(['production_type'=>[1,5]]);
                    ->andFilterWhere(['=','production_type',5]);
            }

        }
        if($this->production_view_type_id == '0' || $this->production_view_type_id == 0){
            $query->andFilterWhere(['production_type'=>[1,5]]);
        }else{
            $query->andFilterWhere(['=','production_type',$this->production_view_type_id]);
        }
