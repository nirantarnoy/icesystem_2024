...
class DailysummaryController extends Controller
{
    ...
    function getProdDaily($product_id, $user_login_datetime, $company_id, $branch_id, $user_id)
    {
        ...
        return $qty==null?0:$qty-$cancel_qty;
    ...
    function getProdRepackDaily($product_id, $user_login_datetime, $user_id)
    {
        $qty = 0;
        if ($product_id != null) {
            $qty = \backend\models\Stocktrans::find()->where(['in', 'activity_type_id', [27]])->andFilterWhere(['product_id' => $product_id, 'created_by' => $user_id])->andFilterWhere(['between', 'trans_date', date('Y-m-d H:i:s', strtotime($user_login_datetime)), date('Y-m-d H:i:s')])->andFilterWhere(['!=','status',500])->andFilterWhere(['>','trans_ref_id',0])->sum('qty');
        }
