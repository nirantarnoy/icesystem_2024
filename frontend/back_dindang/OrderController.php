...
//        // $line_token = 'N3x9CANrOE3qjoAejRBLjrJ7FhLuTBPFuC9ToXh0szh'
        ...
        $message = '' . "\n";
        $message .= 'DD:' . \backend\models\Deliveryroute::findName($route_id) . "\n";
...
//                                }
    ...
    public function actionCancelposorder()
    {
        ...
        if ($order_id != null) {
            $model = \backend\models\Orders::find()->where(['id' => $order_id])->andFilterWhere(['!=','status',3])->one();
            if ($model) {
...
//                                    }
    ...
    public function updatestockcancel($product_id, $qty, $wh_id, $journal_no, $company_id, $branch_id, $user_id)
    {
        if ($product_id != null && $qty > 0) {
            ...
            $model_trans = new \backend\models\Stocktrans();
            $model_trans->journal_no = $last_no;
            $model_trans->trans_date = date('Y-m-d H:i:s');
            $model_trans->product_id = $product_id;
...
//                    }
 ...
 //                   $model_trans->credit_qty = $list_product_data[$i]['line_credit_qty'];//$new_line_credit_qty;
                    ...
                    $model_trans->issue_car_qty = ($carIssueQty - $carIssueOtherQty);
                    $model_trans->issue_transfer_qty = $carIssueOtherQty; //$list_product_data[$i]['line_transfer_qty'];// $this->getTransferout($value->product_id, $cal_date);
 ...
 //       }
        ...
        if ($product_id != null) {
            ...
            if (count($second_user_id) > 0) {
                $qty = \backend\models\Stocktrans::find()->where(['activity_type_id' => 15, 'production_type' => [1,2], 'company_id' => $company_id, 'branch_id' => $branch_id, 'created_by' => $second_user_id])->andFilterWhere(['product_id' => $product_id])->andFilterWhere(['between', 'trans_date', date('Y-m-d H:i:s', strtotime($user_login_datetime)), date('Y-m-d H:i:s')])->sum('qty');
