...
class CustomerController extends Controller
{
    ...
    public function actionBootlist()
    {
        ...
        if ($company_id && $branch_id) {
            $model = \common\models\Customer::find()->where(['company_id' => $company_id, 'branch_id' => $branch_id, 'name' => 'สด-หน้าบ้าน','status'=>1])->all();
            if ($model) {
                $status = true;
                foreach ($model as $value) {
                    array_push($data, [
    ...
    public function actionPoslist()
    {
        ...
        $status = false;
        if ($company_id && $branch_id) {
            $model = \common\models\Customer::find()->where(['company_id' => $company_id, 'branch_id' => $branch_id, 'is_show_pos' => 1,'status'=>1])->all();
            if ($model) {
                $status = true;
                foreach ($model as $value) {
                    array_push($data, [
