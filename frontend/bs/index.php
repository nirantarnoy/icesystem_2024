...
// ฟังก์ชันหลักสำหรับ print PDF จาก iframe
...
// ฟังก์ชันสำหรับ print ทั้ง 2 iframe แบบมี delay
function printBothDocuments() {
    var fileSO = document.querySelector('.slip-print').value;
    var fileDO = document.querySelector('.slip-print-do').value;
    
    
    // Print iframe แรก (DO)
    if (fileSO && fileSO !== "empty") {
        console.log('Printing SO document...');
        printIframe('iFramePdf');
        
        // ถ้ามี iframe ที่ 2 ให้รอแล้ว print
        if (fileDO && fileDO !== "empty") {
            setTimeout(function() {
                console.log('Printing DO Position document...');
                printIframe('iFramePdfDo');
            }, 3000); // delay 3 วินาที
        }
    } 
    // ถ้าไม่มี iframe แรก แต่มี iframe ที่ 2
    // else if (fileCarPos && fileCarPos !== "empty") {
    //     console.log('Printing Car Position document only...');
    //     printIframe('iFramePdfCarPos');
    // }
}

// เรียกใช้เมื่อต้องการ print
// Option 1: เรียกอัตโนมัติเมื่อโหลดหน้าเสร็จ
window.onload = function() {
    // รอให้ iframe โหลด PDF เสร็จก่อน
    setTimeout(function() {
        printBothDocuments();
    }, 2000); // รอ 2 วินาทีหลังโหลดหน้า
};
