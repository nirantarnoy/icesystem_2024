...
//        echo '111';
    ...
    public function actionDeletecusrequestdoc($id){
        //$id = \Yii::$app->request->post('id');
        $res = 0;
        if($id){
            $model = \common\models\CustomerRequestDoc::find()->where(['id'=>$id])->one();
            if($model){
                $file_name = $model->doc_name;
                if (file_exists(\Yii::getAlias('@backend') . '/web/uploads/files/customerrequest/' . $file_name)) {
                    unlink(\Yii::getAlias('@backend') . '/web/uploads/files/customerrequest/' . $file_name);
                    $model->delete();
                    $res = 1;
                }else{
                    if (file_exists(\Yii::getAlias('@backend') . '/web/uploads/files/customers/' . $file_name)) {
                        unlink(\Yii::getAlias('@backend') . '/web/uploads/files/customers/' . $file_name);
                        $model->delete();
                       $res = 1;
                    }
                }
