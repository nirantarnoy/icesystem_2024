...
//            }
            ...
            $f_date = isset($fx_datetime[0]) ? $fx_datetime[0] : null;
            $t_date = isset($tx_datetime[0]) ? $tx_datetime[0] : null;

            $f_time = isset($fx_datetime[1]) ? $fx_datetime[1] : '00:00:00';
            $t_time = isset($tx_datetime[1]) ? $tx_datetime[1] : '23:59:59';

            if ($f_date && $t_date) {
                $from_date_time = date('Y-m-d H:i:s', strtotime($f_date . ' ' . $f_time));
                $to_date_time = date('Y-m-d H:i:s', strtotime($t_date . ' ' . $t_time));
