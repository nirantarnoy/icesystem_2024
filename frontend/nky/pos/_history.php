...
//                ],
                ...
                [
                    ...
                    'contentOptions' => ['style' => 'text-align: center'],
                    'template' => '{print}{reprinthistory}{cancelorder}',
                    'buttons' => [
                        ...
                        'reprinthistory' => function ($url, $data, $index) {
                            ...
                            $options = [
                                ...
                                'data-pjax' => '0',
                                'data-id' => $data->id,
                                'onclick' => 'showReprintHistory(' . $data->id . ')',
                                'style' => 'margin-left: 5px; margin-right: 5px;',
                            ];
                            return Html::a(
                                '<span class="fas fa-history btn btn-xs btn-info"></span>', 'javascript:void(0)', $options);
                        },
...
<div id="reprintHistoryModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-md">
        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header" style="background-color: #17a2b8; color: white;">
                <h4 class="modal-title" style="color: white;"><i class="fas fa-history"></i> ประวัติการพิมพ์ซ้ำ (Reprint History)</h4>
                <button type="button" class="close" data-dismiss="modal" style="color: white; opacity: 1;">&times;</button>
            </div>
            <div class="modal-body">
                <div class="alert alert-info" style="margin-bottom: 15px;">
                    จำนวนการพิมพ์ซ้ำทั้งหมด: <strong><span id="reprint-total-count">0</span></strong> ครั้ง
                </div>
                <table class="table table-bordered table-striped table-reprint-list" width="100%">
                    <thead>
                    <tr>
                        <th style="text-align: center; width: 15%">ลำดับ</th>
                        <th>ผู้พิมพ์ซ้ำ</th>
                        <th style="text-align: center;">วันที่/เวลาพิมพ์</th>
                    </tr>
                    </thead>
                    <tbody>
                    </tbody>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">
                    <i class="fa fa-close text-danger"></i> ปิดหน้าต่าง
                </button>
            </div>
        </div>
    </div>
...
<iframe id="iFramePdf" src="<?= $filename ?>" style="width:0; height:0; border:none; visibility:hidden; position:absolute;"></iframe>
<?php
...
window.reprintOrder = function(id) {
    $.ajax({
        url: 'index.php?r=pos/print',
        type: 'GET',
        data: { id: id },
        dataType: 'json',
...
window.showReprintHistory = function(id) {
    $.ajax({
        url: 'index.php?r=pos/reprinthistory',
        type: 'POST',
        data: { order_id: id },
        dataType: 'json',
        success: function(data) {
            var html = '';
            if (data.length > 0) {
                $("#reprint-total-count").text(data.length);
                $.each(data, function(index, item) {
                    html += '<tr>';
                    html += '<td style="text-align: center;">' + item.index + '</td>';
                    html += '<td>' + item.username + '</td>';
                    html += '<td style="text-align: center;">' + item.printed_at + '</td>';
                    html += '</tr>';
                });
            } else {
                $("#reprint-total-count").text(0);
                html = '<tr><td colspan="3" style="text-align: center; color: red;">ไม่พบประวัติการพิมพ์ซ้ำ</td></tr>';
            }
            $(".table-reprint-list tbody").html(html);
            $("#reprintHistoryModal").modal("show");
        },
        error: function() {
            alert('ไม่สามารถดึงข้อมูลประวัติการพิมพ์ได้');
        }
    });
};
JS;
