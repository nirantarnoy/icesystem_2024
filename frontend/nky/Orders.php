...
//            ],
    ...
    public static function getLastNo($date, $company_id, $branch_id)
    {
        ...
        $model = Orders::find()
            ...
            ->andFilterWhere(['company_id' => 1, 'branch_id' => 1])
...
//         $model = Orders::find()->where(['date(order_date)' => $t_date])->andFilterWhere(['company_id' => $company_id, 'branch_id' => $branch_id])->andFilterWhere(['like', 'order_no', 'SO'])->MAX('order_no');

//         $pre = "SO";
//         if ($model != null) {
//             $prefix = $pre . '-' . date('y', strtotime($date)) . date('m', strtotime($date)) . date('d', strtotime($date)) . '-';
