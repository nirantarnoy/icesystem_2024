...
$is_first_30_days = isset($is_first_30_days) ? $is_first_30_days : 0;
...
// end check date
$customer_active_date = null;
if ($find_customer_id != null) {
    $cust = \backend\models\Customer::find()->where(['id' => $find_customer_id])->one();
    if ($cust) {
        $customer_active_date = $cust->active_date;
    }
}

if ($is_first_30_days == 1 && $customer_active_date != null) {
    $sale_days = \common\models\QueryOrderCustomerCarProduct::find()
        ->select(['DATE(order_date) as sale_date'])
        ->where(['customer_id' => $find_customer_id])
        ->andWhere(['>=', 'order_date', $customer_active_date])
        ->andWhere(['>', 'line_total', 0])
        ->groupBy('DATE(order_date)')
        ->orderBy(['DATE(order_date)' => SORT_ASC])
        ->limit(30)
        ->asArray()
        ->all();

    if (count($sale_days) < 30) {
        $from_date = '1970-01-01 00:00:00';
        $to_date = '1970-01-01 00:00:00';
    } else {
        $from_date = $customer_active_date . ' 00:00:00';
        $to_date = $sale_days[29]['sale_date'] . ' 23:59:59';
    }
}
?>
<html>
...
<form action="<?= \yii\helpers\Url::to(['salereport/indexcar'], true) ?>" method="post">
    <div class="row">
        ...
        <div class="col-lg-3">
            ...
            if ($find_customer_id != null) {
                $cust = \backend\models\Customer::find()->where(['id' => $find_customer_id])->one();
                if ($cust && $cust->active_date != null) {
                    $sale_days_count = \common\models\QueryOrderCustomerCarProduct::find()
                        ->where(['customer_id' => $find_customer_id])
                        ->andWhere(['>=', 'order_date', $cust->active_date])
                        ->andWhere(['>', 'line_total', 0])
                        ->select('DATE(order_date)')
                        ->distinct()
                        ->count();
                    if ($sale_days_count >= 30) {
                        $show_option = true;
                    }
                }
            }
            ?>
            <?php if ($show_option): ?>
                <input type="checkbox" name="is_first_30_days" value="1" <?= $is_first_30_days == 1 ? 'checked' : '' ?>>
                <label for="">ดูข้อมูลการขาย 30 วันแรก</label>
            <?php endif; ?>
...
<div id="div1">
    ...
    if ($find_customer_id != null || $find_customer_id != '') {
        ...
        if ($is_first_30_days == 1) {
            $modelx->andFilterWhere(['>', 'line_total', 0]);
        }
        
        $modelx = $modelx->groupBy('product_id')->orderBy(['product_id' => SORT_ASC])->all();
        ...
        $model_line = \common\models\QueryOrderCustomerCarProduct::find()->select(['id', 'order_no'])->where(['customer_id' => $find_customer_id])
            ->andFilterWhere(['BETWEEN', 'order_date', $from_date, $to_date])->andFilterWhere(['>','qty',0]);
        
        if ($is_first_30_days == 1) {
            $model_line->andFilterWhere(['>', 'line_total', 0]);
        }
            ...
            <?php foreach ($model_line as $value): ?>
                ...
                <tr>
                    ...
                    <?php for ($x = 0; $x <= count($product_header) - 1; $x++): ?>
                        <?php
                        $product_line_qty = getOrderQty2($value->id, $product_header[$x], $is_first_30_days);
                        $product_line_amt = getOrderAmount($value->id, $product_header[$x], $is_first_30_days);
...
<?php
function getOrderQty($order_id, $is_first_30_days = 0)
{
    $qty = 0;
    if ($order_id) {
        $query = \backend\models\Orderline::find()->where(['order_id' => $order_id]);
        if ($is_first_30_days == 1) {
            $query->andFilterWhere(['>', 'line_total', 0]);
        }
        $model_qty = $query->sum('qty');
        if ($model_qty) {
            $qty = $model_qty;
        }
    }
...
function getOrderQty2($order_id, $product_id, $is_first_30_days = 0)
{
    $data = 0;
    if ($order_id) {
        $query = \backend\models\Orderline::find()->where(['order_id' => $order_id, 'product_id' => $product_id]);
        if ($is_first_30_days == 1) {
            $query->andFilterWhere(['>', 'line_total', 0]);
        }
        $model_qty = $query->sum('qty');
...
function getOrderAmount($order_id, $product_id, $is_first_30_days = 0)
{
    $data = 0;
    if ($order_id) {
        $query = \backend\models\Orderline::find()->where(['order_id' => $order_id, 'product_id' => $product_id]);
        ...
        $model_amount = $query->sum('line_total');
        if ($model_amount) {
            $data = $model_amount;
//           foreach($model_qty as $value){
//            //   $name = \backend\models\Product::findCode($value->product_id);
