...
class AdminreportController extends Controller
{
    ...
    public function actionEditsummary()
    {
        ...
        if($prod_rec_qty == null && $transfer_qty == null && $return_qty == null && $scrap_qty == null && $counting_qty == null && $refill_qty == null && $reprocess_qty == null && $balance_in_qty == null && $sale_qty == null && $cash_qty == null && $credit_qty == null && $issue_car_qty == null && $issue_transfer_qty == null){
...
//            'is_invoice_req' => $is_invoice_req,
    ...
    public function actionSaveeditqty()
    {
        $trans_edit_date = \Yii::$app->request->post('trans_date');
        $product_id = \Yii::$app->request->post('product_id');
        $new_qty = \Yii::$app->request->post('qty');


        if ($trans_edit_date != null && $product_id) {

            $new_qty2 = str_replace(',', '', $new_qty);
            $check_has = \common\models\CloseMonthlyQuantity::find()->where(['year(trans_date)' => date('Y', strtotime($trans_edit_date)), 'month(trans_date)' => date('m', strtotime($trans_edit_date)), 'product_id' => $product_id])->one();
            if ($check_has) {
                $check_has->qty = $new_qty2;
                $check_has->save(false);
            } else {
                $model = new \common\models\CloseMonthlyQuantity();
                $model->trans_date = date('Y-m-d', strtotime($trans_edit_date));
        ...
        return $this->render('_print_sale_all_summary', [
            'from_date' => date('Y-m-d', strtotime($trans_edit_date)),
            'company_id' => $company_id,
            'branch_id' => $branch_id,
            'btn_order_type' => null,
        ]);
    ...
    public function actionPrintcarlotus()
    {
        $from_date = \Yii::$app->request->post('from_date');
        $to_date = \Yii::$app->request->post('to_date');
        $route_id = \Yii::$app->request->post('route_id');
        ...
        return $this->render('_cardaily_lotus',[
            'from_date' => $from_date,
            'to_date' => $to_date,
            'route_id' => $route_id,
            'product_id' => $product_id,
    ...
    public function actionCustomerMonthlyReport()
    {
        ...
        foreach ($route_id as $i => $rid) {
            $key = ":r{$i}";
            $routePlaceholders[] = $key;
            $params[$key] = $rid;
        }

        foreach ($customer_group_id as $j => $gid) {
            $key = ":g{$j}";
