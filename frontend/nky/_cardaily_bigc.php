...
$bigc_group_id = 0;
$m_bigc = \backend\models\Customergroup::find()->where(['like', 'name', 'BigC'])->one();
if($m_bigc){
    $bigc_group_id = $m_bigc->id;
}

$model_cj = null;
$sql = "SELECT customer_code,customer_name,route_code,branch_no,product_id,product.name as prd_name FROM query_order_bigc_summary left join product on query_order_bigc_summary.product_id = product.id";
...
if($bigc_group_id > 0){
    $sql .= " AND query_order_bigc_summary.customer_group_id = '" . $bigc_group_id . "'";
}
...
$model_cj_data = null;
$sql = "SELECT * FROM query_order_bigc_summary";
$sql .= " WHERE (date(order_date) BETWEEN '" . $from_date . "' AND '" . $to_date . "')";
if($route_id!=null){
...
if($bigc_group_id > 0){
    $sql .= " AND customer_group_id = '" . $bigc_group_id . "'";
}
...
<div class="row">
    <div class="col-lg-9">
        <form action="<?= \yii\helpers\Url::to(['adminreport/printcarbigc'], true) ?>" method="post" id="form-search">
...
<div id="div1">
    <table class="table-header" width="100%">
        <tr>
            <td style="text-align: center; font-size: 20px; font-weight: bold">รายงานจำนวนขายสายส่ง BigC</td>
        </tr>
    </table>
    <br>
    <table class="table-header" width="100%">
