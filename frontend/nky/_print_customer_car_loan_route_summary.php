        ...
        <table class="table-header" style="width: 100%;font-size: 18px;" border="0">
            ...
            <tr>
                ...
                <td style="width: 20%">
                    ...
                    <select name="find_has_detail" class="form-control" id="find_has_detail" onchange="checkHasDetail($(this))">
                ...
                <td style="width: 15%">
                    <label for="">เครื่องหมาย</label>
                    <?php
                    $is_enabled = ($find_has_detail == 1);
                    echo \kartik\select2\Select2::widget([
                        'name' => 'find_mark',
                        'data' => [
                            'All' => 'All',
...
<table class="table-title" id="table-data" style="width: 100%">
    ...
    <?php if ($model_customer_loan != null): ?>
        <?php foreach ($model_customer_loan as $value): ?>
            ...
            $monthly_data = getRouteMonthlySummary($value->route_id, $from_date, $to_date, $company_id, $branch_id, $is_find_date, $find_customer_id_select, $find_has_detail, $find_mark);
...
<?php
function getRouteMonthlySummary($route_id, $f_date, $t_date, $company_id, $branch_id, $is_find_date, $find_customer_id_select, $find_has_detail, $find_mark)
{
    $data = [];
    if ($route_id != null) {
        if ($is_find_date == 0 || $is_find_date == null) {
            $sql = "SELECT 
             ...
             if($find_has_detail == 1 && $find_mark != 'All' && $find_mark != ''){
                 if($find_mark == '*'){
                     $sql .= " AND t3.name LIKE '%*%' AND t3.name NOT LIKE '%**%'";
                 }else if($find_mark == '**'){
                     $sql .= " AND t3.name LIKE '%**%' AND t3.name NOT LIKE '%***%'";
                 }else if($find_mark == '***'){
                     ...
                     $sql .= " AND t3.name LIKE '%" . $find_mark . "%'";
                 }
             }
...
function checkHasDetail(e){
    var val = e.val();
    if(val == 1){
        $("#find_mark").prop("disabled", false);
    }else{
        $("#find_mark").val("All").trigger("change");
        $("#find_mark").prop("disabled", true);
    }
}
JS;
