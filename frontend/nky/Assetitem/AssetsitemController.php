...
class AssetsitemController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $uploadPath = \Yii::getAlias('@frontend/web/uploads/assetphoto/');
...
//                echo count($customer_form); return ;
                    ...
                    foreach ($asset_photo as $value) {
                        ...
                        $value->saveAs($uploadPath . $file_name);
    ...
    public function actionDeletephoto(){
        ...
        $uploadPath = \Yii::getAlias('@frontend/web/uploads/assetphoto/');
        if($id){
            $model = \common\models\AssetsPhoto::find()->where(['id'=>$id,'asset_id'=>$asset_id])->one();
            if($model){
                if(file_exists($uploadPath.$model->photo)){
                    unlink($uploadPath.$model->photo);
                }
                $model->delete();
            }
        }
        return $this->redirect(['update','id'=>$asset_id]);
    }
...
//        }
    ...
    public function actionGetItem()
    {
        ...
        if ($model) {
            foreach ($model as $value) {
                if($this->checkAssetUsed($value->id)){
                    continue; // ไม่สามารถเลือกได้
                }
    ...
    public function checkAssetUsed($id){
        $model_count = \backend\models\Customerasset::find()->where(['product_id' => $id])->count();
        return $model_count;
    }
