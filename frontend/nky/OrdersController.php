...
//        }
   ...
   public function actionCreatemanual(){
        return $this->render('_createmanual');
   }
    public function actionAddordernew() // 29/01/2023 cut transline
    {
        ...
        if ($customer_id != null && $route_id != null) {
            ...
            $model->payment_method_id = 2;// $payment_type_id;
                ...
                if (count($datalist) > 0) {
                    ...
                    for ($i = 0; $i <= count($datalist) - 1; $i++) {
                        ...
                        try {
                            ...
                            $model_line->status = 1;
                            $model_line->sale_payment_method_id = 2;// $payment_type_id;
...
//        }
    ...
    public function actionGetcarbyroute(){
        $id = \Yii::$app->request->post('id');
        $html = 'xxxx';
        if($id){
            $model = \backend\models\Salegroup::find()->where(['delivery_route_id'=>$id])->one();
            if($model){
                $modelx = \backend\models\Car::find()->where(['sale_group_id'=>$model->id])->all();
               // $modelx = \backend\models\Car::find()->all();
                if($modelx){
                    foreach($modelx as $value){
    ...
    public function actionGetcustomerbyroute(){
        $id = \Yii::$app->request->post('id');
        $html = '';
        if($id){
            $model = \backend\models\Customer::find()->where(['delivery_route_id'=>$id])->all();
            if($model){
                    foreach($model as $value){
                        $html.='<option value="'.$value->id.'">';
                        $html.= $value->name;
