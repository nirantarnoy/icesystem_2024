...
//$model_line = \common\models\QuerySaleorderByRoute::find()->select(['customer_id'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
//   ->andFilterWhere(['rt_id' => $find_route])->orderBy(['order_date' => SORT_ASC])->groupBy('customer_id')->all();
$model_line = \common\models\Customer::find()->select(['id'])->where(['status' => 1,'delivery_route_id'=>$find_route])
...
<div class="row">
    <div class="col-lg-9">
        <form action="<?= \yii\helpers\Url::to(['saleroutesummary/index'], true) ?>" method="post"
              id="form-search">
                ...
                <tr>
                    ...
                    <td style="width: 20%">
                        ...
                        echo \kartik\select2\Select2::widget([
                            'name' => 'find_route',
                            'data' => \yii\helpers\ArrayHelper::map(\backend\models\Deliveryroute::find()->where(['status' => 1, 'is_other_branch' => 0])->all(), 'id', 'name'),
                            'value'=> $find_route,
                            'options' => [
...
<div id="div1">
    <table class="table-header" width="100%">
        <tr>
            <td style="text-align: center; font-size: 20px; font-weight: bold">รายงานแสดงลูกค้าตามสายส่ง</td>
        </tr>
    </table>
    <br>
    <table class="table-header" width="100%">
    ...
    $product_header = [];
    //
    //    $modelx = \common\models\QuerySaleByDistributor::find()->select(['product_id'])->join('inner join', 'product', 'query_sale_by_distributor.product_id=product.id')->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($$
    ...
    $sql = "SELECT product_id FROM query_sale_by_route INNER JOIN product ON query_sale_by_route.product_id = product.id";
    $sql .= " WHERE query_sale_by_route.company_id=" . $company_id . " AND query_sale_by_route.branch_id=" . $branch_id;
...
//        }
    ...
    <table id="table-data">
        <tr style="font-weight: bold;">
            ...
            <td style="text-align: center;padding: 8px;border: 1px solid grey;">Route Num</td>
        ...
        <?php
        $selected_route = ''; //\backend\models\Deliveryroute::findName($find_route);
        $line_num = 0;
        ?>
...
//              $total_all_line_qty = 0;
            ...
            ?>
            <tr>
                <td style="text-align: center;padding: 8px;border: 1px solid grey"><?= $line_num ?></td>
                <td style="text-align: left;padding: 8px;border: 1px solid grey"><?= \backend\models\Customer::findName($value->id) ?></td>
                <td style="text-align: center;padding: 8px;border: 1px solid grey"><?= \backend\models\Customer::findRouteNum($value->id) ?></td>
                <?php
                $product_line_free_qty =  getFree($value->id, $from_date, $to_date);
                ...
                $product_line_receive_transfer =  0;// getReceiveTransfer($value->id, $from_date, $to_date);
                ...
                ?>
                <?php
                   $product_line_data = getOrderQty2($value->id, $from_date, $to_date);
                   $customer_cash_data = getCashAmount($value->id, $from_date, $to_date);
                ?>
                <?php for ($x = 0; $x <= count($product_header) - 1; $x++): ?>
                    <?php
                    $product_line_qty = 0  ;// getOrderQty2($value->id, $product_header[$x], $from_date, $to_date);
                    if($product_line_data!=null){
                        for($i=0;$i<count($product_line_data);$i++){
                            if($product_line_data[$i]['product_id']==$product_header[$x]){
                                $product_line_qty = $product_line_data[$i]['qty'];
                            }
                        }
                    }
                    ...
                    $product_line_credit_amount = 0; // getCreditAmount($value->id, $product_header[$x], $from_date, $to_date);
                ...
                <?php endfor; ?>
                <?php
//                $product_line_cash_amount = $customer_cash_data;
                $line_cash_amount_total = ($line_cash_amount_total + $customer_cash_data);
                $all_cash = ($all_cash + $customer_cash_data);
                ?>
        ...
        <tfoot>
        <tr>
            <td colspan="3" style="text-align: right;padding: 5px;border: 0px solid grey"><b>รวม</b></td>
...
<?php
function getOrderQty2($customer_id, $from_date, $to_date)
{
    $data = [];
    if ($customer_id) {
        $model_qty = \common\models\QuerySaleMobileDataNew::find()->select(['product_id','SUM(line_qty_credit) as line_qty_credit', 'SUM(line_qty_cash) as line_qty_cash'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])->andFilterWhere(['customer_id' => $customer_id])->groupBy(['product_id'])->all();
        if ($model_qty != null) {
            foreach($model_qty as $value){
                array_push($data,['product_id'=>$value->product_id,'qty'=>$value->line_qty_credit+$value->line_qty_cash]);
            }
        }
    }
    return $data;
...
function getFree($customer_id, $from_date, $to_date)
{
    $data = 0;
    if ($customer_id) {
        $model_qty = \common\models\QuerySaleMobileDataNew::find()->select(['SUM(line_qty_free) as line_qty_free'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['customer_id' => $customer_id])->groupBy(['customer_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->line_qty_free);
        }
    }
    return $data;
}
...
function getCashAmount($customer_id, $from_date, $to_date)
{
    $data = 0;
    if ($customer_id) {
        $model_qty = \common\models\QuerySaleMobileDataNew::find()->select(['SUM(line_total_cash) as line_total_cash'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['customer_id' => $customer_id])->groupBy(['customer_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->line_total_cash);
        }
    }
    return $data;
}
...
function getCreditAmount($customer_id, $product_id, $from_date, $to_date)
{
    ...
    if ($customer_id && $product_id) {
        $model_qty = \common\models\QuerySaleMobileDataNew::find()->select(['SUM(line_total_credit) as line_total_credit'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['customer_id' => $customer_id, 'product_id' => $product_id])->groupBy(['product_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->line_total_credit);
        }
    }
    return $data;
}
...
//        }
        $model_qty = \common\models\QuerySaleCustomerPaySummary::find()
            ->where(['BETWEEN', 'date(payment_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['customer_id' => $customer_id,'pay_type'=>1])->sum('payment_amount');
        if ($model_qty != null) {
            $data = ($model_qty);
        }
    }
...
//    }
    $model_qty = \common\models\TransactionCarSaleRoutePay::find()
        ->where(['BETWEEN', 'date(QuerySaleCustomerPaySummary)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
        ->andFilterWhere(['customer_id' => $customer_id,'pay_type'=>2])->sum('payment_amount');
    if ($model_qty != null) {
        $data = ($model_qty);
    }
    return $data;
