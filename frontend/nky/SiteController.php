...
//                    ],
                    [
                        'actions' => ['login', 'error', 'createadmin', 'changepassword', 'decodex', 'grab', 'addseconduser', 'getcominfo', 'transactionsalecar', 'transactionsalecar2', 'transactionsalecar3', 'createscreenshort', 'transactionsalepos',
                            'updateroute', 'calmachine', 'clearorder', 'testclosesum', 'updateorderpayment', 'caltransactionsaledistributor',
                            'startcaldailymanagerauto', 'uploadfromgo', 'transfercustomerinfo','generate-customer-monthly-sum'],
                        'allow' => true,
                    ],
                    [
...
//            ]);
    ...
    function actionTransfercustomerinfo()
    {
         $model = \backend\models\Customerrequest::find()->all();
         $xcount = 0;
         if($model){
             foreach ($model as $value) {
                 $customer = \common\models\Customer::find()->where(['id' => $value->customer_ref_id])->one();
                 if($customer) {
                     if($customer->idcard_no == null || $customer->idcard_no == ''){
                         $customer->idcard_no = $value->idcard_no;
