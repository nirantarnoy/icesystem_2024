...
//        }
        ...
        $has_driver = 1;
        if (isset(Yii::$app->request->queryParams['CardailySearch']['has_driver'])) {
            $has_driver = Yii::$app->request->queryParams['CardailySearch']['has_driver'];
        }
        $searchModel->has_driver = $has_driver;
        ...
        if($has_driver == 1){
             $query->andFilterWhere(['id' => \backend\models\Cardaily::find()->select('car_id')->where(['date(trans_date)' => $searchModel->trans_date, 'company_id' => $company_id, 'branch_id' => $branch_id])]);
        }else if($has_driver == 2){
             $query->andFilterWhere(['NOT IN', 'id', \backend\models\Cardaily::find()->select('car_id')->where(['date(trans_date)' => $searchModel->trans_date, 'company_id' => $company_id, 'branch_id' => $branch_id])]);
        }
