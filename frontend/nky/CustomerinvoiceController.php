...
//            $sql .= " AND date(t1.order_date) >='". date('Y-m-d',strtotime($pre_date))."'";
            ...
            if($f_date != '' && $t_date != ''){
             ...
             $sql .= " GROUP BY t1.id";
             $sql .= " ORDER BY t1.order_date ASC";
...
//                    $html .= '<td style="text-align: center"><input type="file" class="form-control"></td>';
    ...
    public function actionPrint($id)
    {
        if ($id) {
            ...
            $model = \backend\models\Customerinvoice::find()->where(['id' => $id])->one();
            if ($model) {
                $model_line = \common\models\CustomerInvoiceLine::find()->joinWith('order')->where(['customer_invoice_line.customer_invoice_id' => $id])->orderBy(['orders.order_date' => SORT_ASC])->all();
//                $sql = "SELECT ci ";
//                $sql.="FROM customer_invoice_line as ci INNER JOIN orders as o ON ci.order_id = o.id";
//                $sql.="WHERE ci.customer_invoice_id =".$id;
...
//
   ...
   public function actionRecal($id){
              ...
              foreach($model as $value){
                 ...
                 if($model_order_line){
                     foreach($model_order_line as $value_line){
                         $product_price = $this->findProductPrice($customer_id,$value_line->product_id);
                        $line_amount = ($product_price * $value_line->qty);
                        $order_amount_new += $line_amount;// ($this->findProductPrice($customer_id,$value_line->product_id) * $value_line->qty);

                        $this->updateOrderline($value_line->id,$line_amount,$product_price);
                     }
    ...
    public function updateOrderline($id,$new_amount,$product_price){
        if($id && $new_amount){
            $model = \backend\models\Orderline::find()->where(['id'=>$id])->one();
            if($model){
                $model->price = (double)$product_price;
                $model->line_total = (double)$new_amount;
                $model->save(false);
            }
        }
    }
