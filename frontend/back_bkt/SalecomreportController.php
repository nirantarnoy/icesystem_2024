...
//        if($model_route_all){
//            $total_after_check_is_cal_count = 0;
//            foreach ($model_route_all as $valuexx2){
//                $model_route_not_cal = \common\models\DeliveryRoute::find()->select(['is_cal_commission'])->where(['id'=>$valuexx2->order_channel_id])->one();
//                if($model_route_not_cal->is_cal_commission == 0)continue;
//                $total_after_check_is_cal_count +=1;
//            }
//            foreach ($model_route_all as $valuexx){
//                $model_route_not_cal = \common\models\DeliveryRoute::find()->select(['is_cal_commission'])->where(['id'=>$valuexx->order_channel_id])->one();
//                if($model_route_not_cal->is_cal_commission == 0)continue;
//
...
//                if($total_after_check_is_cal_count == $counted){
...
//        }
        ...
        // ดึง order_channel_id เฉพาะวันที่ตรงแบบใช้ index ได้
        $model_route_all = \common\models\Orders::find()
            ->select(['order_channel_id'])
            ->distinct()
            ->where(['sale_from_mobile' => 1])
            ->andWhere(['date(order_date)' => date('Y-m-d', strtotime($t_date))])
            ->column(); // ดึงเป็น array (ประหยัดหน่วยความจำ)

// ดึงข้อมูล DeliveryRoute ทั้งหมดที่เกี่ยวข้องมาใน query เดียว
        $routes = \common\models\DeliveryRoute::find()
            ->select(['id', 'is_cal_commission'])
            ->where(['id' => $model_route_all])
            ->indexBy('id') // ทำให้เรียกใช้ง่าย $routes[$id]
            ->asArray()
            ->all();

// ดึงรายการที่เคยคำนวณไปแล้ว
        $already_cals = \common\models\ComDailyCal::find()
            ->select('route_id')
            ->distinct()
            ->where(['in', 'route_id', $model_route_all])
            ->andWhere(['date(trans_date)' => date('Y-m-d', strtotime($t_date))])
            ->column(); // array ของ route_id ที่คำนวณแล้ว
...
// ตรวจสอบ
        ...
        foreach ($model_route_all as $channel_id) {
            // ข้ามถ้าไม่มี route หรือไม่ต้องคำนวณ
            if (!isset($routes[$channel_id]) || $routes[$channel_id]['is_cal_commission'] == 0) {
                continue;
            }

            $total_after_check_is_cal_count++;

            if (!in_array($channel_id, $already_cals)) {
                $model_route_data[] = $channel_id;
                $loop++;
                if ($loop == 20) break;
            } else {
                $counted++;
...
// ตรวจสอบว่าคำนวณครบทุกสายหรือยัง
        if ($total_after_check_is_cal_count == $counted) {
            Yii::$app->session->setFlash('msg-already', 'คำนวนครบทุกสายส่งแล้ว');
            return $this->redirect(['salecomreport/index']);
        }
        ...
        if ($model_route_data != null) {
            if(count($model_route_data) >0){
                for($a=0;$a<=count($model_route_data)-1;$a++) {
                  ...
                  //  if($model_route_data[$a] != 959)continue;
