...
//             AND t1.company_id=" . $company_id . " AND t1.branch_id=" . $branch_id;
    ...
    $sql = "SELECT t1.id,t1.slip_doc,t1.slip_doc2,t1.slip_doc3,t1.journal_no,t1.customer_code,t1.customer_name,t1.customer_id,SUM(t1.payment_amount) as amount,t1.trans_date  from query_payment_receive as t1 INNER JOIN customer as t2 on t2.id = t1.customer_id 
...
//                $customer_name = \backend\models\Customer::findName($model[$i]['customer_id']);
            ...
            array_push($data, [
                ...
                'slip_doc2' => $model[$i]['slip_doc2'],
                'slip_doc3' => $model[$i]['slip_doc3'],
            ]);
        }
    }
    return $data;
