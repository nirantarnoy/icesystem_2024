                        ...
                        if ($model->save(false)) {
                            ...
                            if ($model_update_line) {
                                ...
                                if ($model_update_line->save(false)) {
                                    ...
                                    $this->notifymessage('สายส่ง: ' . $route_name . ' ยกเลิกรายการขายเงื่อนไข 1 ' . $order_no . ' ลูกค้า: ' . $customer_code . ' ยอดเงิน: ' . $model->line_total . ' เหตุผล: ' . $reason.' จำนวนคืน '.$model->qty);
                                }
                            }
                        }
                    }
                            ...
                            if ($model->save(false)) {
                                ...
                                if ($model_update_line) {
                                    ...
                                    if ($model_update_line->save(false)) {
                                        $status = 1;
                                        array_push($data, ['cancel_order' => 'successfully']);
                                        $this->notifymessage('สายส่ง: ' . $route_name . ' ยกเลิกรายการขายเงื่อนไข 2 ' . $order_no . ' ลูกค้า: ' . $customer_code . ' ยอดเงิน: ' . $model->line_total . ' เหตุผล: ' . $reason.' จำนวนคืน '.$model->qty);
                                    }
                                }
                            }
                        }
                    ...
                    if ($model_order_stock) {
                       // $model_order_stock->avl_qty = ($model_order_stock->avl_qty + $model->qty);
                        ...
                        if ($model_order_stock->save(false)) {
                            ...
                            if ($model->save(false)) {
                               // $model_update_line = \backend\models\Orderline::find()->where(['order_id' => $model->order_id, 'product_id' => $model->product_id, 'customer_id' => $customer_id])->one();
                                $model_update_line = \backend\models\Orderline::find()->where(['id' => $model->id])->one();
                                if ($model_update_line) {
//                                $new_line_total = ($model_update_line->price * $model->qty);
//                                $model_update_line->qty = ($model_update_line->qty - $model->qty);
//                                $model_update_line->line_total = ($model_update_line->line_total - $new_line_total);
                                    ...
                                    if ($model_update_line->save(false)) {
                                        ...
                                        array_push($data, ['cancel_order' => 'successfully']);
                                        $this->notifymessage('สายส่ง: ' . $route_name . ' ยกเลิกรายการขายเงื่อนไข 1 ' . $order_no . ' ลูกค้า: ' . $customer_code . ' ยอดเงิน: ' . $model->line_total . ' เหตุผล: ' . $reason . ' จำนวน: ' . $model->qty);
                                    }
                                }
                            }
                        }
