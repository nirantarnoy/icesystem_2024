...
if($route_id!=null){
    ...
    $sql .= " AND customer.delivery_route_id in (".$where_item.")";
}
if($product_id!=null){
            ...
            <table class="table-header" style="width: 100%;font-size: 18px;" border="0">
                <tr>
                    ...
                    <td>
                        ...
                        echo \kartik\select2\Select2::widget([
                            'name' => 'product_id',
                            'data' => \yii\helpers\ArrayHelper::map(\backend\models\Product::find()->where(['status'=>1,'id'=>[67,69]])->all(), 'id', 'name'),
                            'value' => $product_id,
                            'options' => [
        ...
        <?php for ($x = 0; $x <= count($model_cj) - 1; $x++): ?>
            ...
            <tr>
                ...
                <?php for ($i = 0; $i <= count($day_arr) - 1; $i++): ?>
                    ...
                    $line_qty_display = '';
                    if($qty <=0){
                        if($checkin_status !=0 && $checkin_status !=2){
                            if($checkin_status==1){
                                $line_qty_display = '<div style="background-color: green; width: 15px; height: 15px; border-radius: 50%; margin: auto;"></div>';
                            }else if($checkin_status==3){
                                $line_qty_display = '<div style="background-color: red; width: 15px; height: 15px; border-radius: 50%; margin: auto;"></div>';
                            }
                        }else{
                            $line_qty_display = '-';
                        }
                        ...
                        $line_qty_display = number_format($qty, 0);
                    }

                    ?>
                    <td style="text-align: center;padding: 8px;border: 1px solid grey;"><?= $line_qty_display ?></td>
                <?php endfor; ?>
...
function getCheckinStatus($customer_id,$route_id,$modelx,$day){
    $status = 0;
    if($modelx){
        $year = (int)date('Y',strtotime($modelx[0]['order_date']));
        $month= (int)date('m',strtotime($modelx[0]['order_date']));
        $model = \common\models\CustomerCheckin::find()->where(['customer_id'=>$customer_id,'route_id'=>$route_id,'year(checkin_date)'=>$year,'month(checkin_date)'=>$month,'day(checkin_date)'=>$day])->orderBy(['id'=>SORT_DESC])->one();
        if($model){
            $status = $model->reason_id;
        }
    }
