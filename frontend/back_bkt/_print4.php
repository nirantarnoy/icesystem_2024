...
<div id="div1">
    ...
    for ($x=0;$x<=count($model_temp_data)-1;$x++) {
        ...
        $sql .= " WHERE  id =" . $model_temp_data[$x]['order_id'];
...
//            }
    ...
    <table id="table-data">
        ...
        <?php for($m=0;$m<=count($model_temp_data)-1;$m++): ?>
            <?php
            $num += 1;

            $total_line = $model_temp_data[$m]['remain_amount']; // \backend\models\Orders::getlineremainsum($value->order_id, $model->customer_id);
            $total_all = $total_all + $total_line;

            $line_qty = getOrderQty($model_temp_data[$m]['order_id']);
            $total_all_line_qty = $total_all_line_qty + $line_qty;
//              $order_product = getOrderQty2($value->order_id);
//              $total_all_line_qty = 0;
            $is_29_02 = 0;
            $find_order_date = date('Y-m-d');
            $find_or_date = \backend\models\Orders::getOrderdate($model_temp_data[$m]['order_id']);
            ...
            <tr>
                ...
                <td style="text-align: center;padding: 8px;border: 1px solid grey"><?= \backend\models\Orders::getCustomerrefno($model_temp_data[$m]['order_id']) ?></td>
                <?php for ($x = 0; $x <= count($product_header) - 1; $x++): ?>
                    <?php
                    $product_line_qty = getOrderQty2($model_temp_data[$m]['order_id'], $product_header[$x]);
                    if ($num == 1) {
                ...
                <td style="text-align: center;padding: 8px;padding-right: 5px;border: 1px solid grey"><?= $model_temp_data[$m]['note'] ?></td>
            </tr>
        <?php endfor; ?>
        <tfoot>
        <tr>
