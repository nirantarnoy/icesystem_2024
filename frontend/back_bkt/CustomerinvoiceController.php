...
//                    $html .= '<td style="text-align: center"><input type="file" class="form-control"></td>';
    ...
    public function actionPrint($id)
    {
        if ($id) {
            ...
            if ($model) {
                ...
                $model_line = \common\models\CustomerInvoiceLine::find()
                    ->alias('cil') // ตั้ง alias ให้ table หลัก
                    ->joinWith(['order o']) // join relation 'order' ที่ต้องกำหนดไว้ใน model
                    ->where(['cil.customer_invoice_id' => $id])
                    ->orderBy(['o.order_date' => SORT_ASC]) // sort จากตาราง orders
                    ->all();
                //  $model_line = \common\models\CustomerInvoiceLine::find()->where(['customer_invoice_id' => $id])->orderBy(['trans_date' => SORT_ASC])->all();
                if ($model_line) {
                    foreach ($model_line as $value) {
    ...
    public function actionPrintXX($id)
    {
        if ($id) {
            $model_line = null;
            $model_min_max = [];
            ...
            if ($model) {
                ...
                if ($model_line) {
                    foreach ($model_line as $value) {
                        ...
                        $order_date_new = date('Y-m-d', strtotime(\backend\models\Orders::getOrderdate($value->order_id)));
                        array_push($model_temp_data, [
            ...
            if($model_temp_data!=null){
                usort($model_temp_data, function($a, $b) {
                    return strtotime($a['order_date']) - strtotime($b['order_date']);
                });
            }
            return $this->render('_print4', [
                'model' => $model,
                'model_line' => $model_line,
                'model_min_max' => $model_min_max,
                'model_temp_data' => $model_temp_data,
            ]);
        } else {
            return $this->redirect('customerinvoice/index');
        }
