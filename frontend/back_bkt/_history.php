...
//                ],
                ...
                [
                    ...
                    'contentOptions' => ['style' => 'text-align: center'],
                    'template' => '{print}{cancelorder}{attachslip}',
                    'buttons' => [
...
//                                'target' => '_blank',
                        ...
                        'attachslip' => function ($url, $data, $index) {
                            $has_slip = false;
                            $extensions = ['jpg', 'jpeg', 'png', 'gif'];
                            foreach ($extensions as $ext) {
                                $filePath = \Yii::getAlias('@webroot') . '/uploads/company2/slip/slip_' . $data->id . '.' . $ext;
                            ...
                            $btn_class = $has_slip ? 'btn-success' : 'btn-default';
                            $icon_class = $has_slip ? 'fas fa-check' : 'fas fa-paperclip';
                            $title = $has_slip ? 'มีสลิปแล้ว' : 'แนบสลิป';
                            ...
                            $options = [
                                ...
                                'data-pjax' => '0',
                                'data-id' => $data->id,
                                'onclick' => 'showAttachSlip($(this))'
                            ];
                            return Html::a(
                                '<span class="'.$icon_class.' btn btn-xs '.$btn_class.'"></span>', 'javascript:void(0)', $options);
                        },
                    ]
                ],
            ],
...
<div id="modalAttachSlip" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form-upload-slip" action="<?= \yii\helpers\Url::to(['pos/uploadslip'], true) ?>" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="order_id" class="upload-order-id" value="">
                    <div class="form-group">
                        <label for="">เลือกไฟล์รูปภาพ</label>
                        <input type="file" name="slip_file" class="form-control" accept="image/*" required>
                    </div>
                    <div class="form-group text-center">
                         <div id="preview-slip-area"></div>
                    </div>
...
function showAttachSlip(e){
    var id = e.attr("data-id");
    $(".upload-order-id").val(id);
    $("#preview-slip-area").html(''); // Clear previous preview
    
    // Check if file exists (optional, requires ajax to check)
    $.ajax({
        url: 'index.php?r=pos/getslip',
        type: 'POST',
        data: {id: id},
        success: function(data){
            if(data){
                $("#preview-slip-area").html('<img src="'+data+'" style="max-width: 100%; max-height: 300px;">');
            }
        }
    });

    $("#modalAttachSlip").modal("show");
}     
JS;
