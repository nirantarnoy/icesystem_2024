...
//                }
    ...
    public function actionAddpay2()
    {
        ...
        $payment_channel_id = isset($req_data['payment_channel_id']) ? $req_data['payment_channel_id'] : 1; // 1 cash 2 transfer
        $pay_date = isset($req_data['pay_date']) ? $req_data['pay_date'] : date('d-m-Y');
        $company_id = isset($req_data['company_id']) ? $req_data['company_id'] : null;
        $branch_id = isset($req_data['branch_id']) ? $req_data['branch_id'] : null;
        $user_id = isset($req_data['user_id']) ? $req_data['user_id'] : null;
        $data_list = isset($req_data['data']) ? $req_data['data'] : null;

        $base64_string = isset($req_data['image']) ? $req_data['image'] : null;
        $base64_string2 = isset($req_data['image2']) ? $req_data['image2'] : null;
        $base64_string3 = isset($req_data['image3']) ? $req_data['image3'] : null;
        ...
        if ($company_id != null && $branch_id != null && $pay_date != null && $data_list != null && $user_id != null) {
            if (count($data_list) > 0) {
                $model = null;
                $payment_receive_id = 0;

                for ($i = 0; $i <= count($data_list) - 1; $i++) {
                    if (!isset($data_list[$i]['order_id']) || !isset($data_list[$i]['pay_amount']) || $data_list[$i]['pay_amount'] <= 0) continue;

                    if ($model == null) {
                        $customer_id = $data_list[$i]['customer_id'];
                        $model = new \backend\models\Paymentreceive();
                        $model->trans_date = date('Y-m-d H:i:s', strtotime($t_date . ' ' . date('H:i:s')));
                        $model->customer_id = $customer_id;
                        $model->journal_no = $model->getLastNo2(date('Y-m-d'), $company_id, $branch_id);
                        $model->status = 1;
                        $model->company_id = $company_id;
                        ...
                        if ($base64_string2 != null) {
                            $newfile2 = time() . "_2.jpg";
                            $outputfile2 = '../web/uploads/files/receive/' . $newfile2;
                            $filehandler2 = fopen($outputfile2, 'wb');
                            fwrite($filehandler2, base64_decode(trim($base64_string2)));
                            fclose($filehandler2);
                            $model->slip_doc2 = $newfile2;
                        }
                        if ($base64_string3 != null) {
                            $newfile3 = time() . "_3.jpg";
                            $outputfile3 = '../web/uploads/files/receive/' . $newfile3;
                            $filehandler3 = fopen($outputfile3, 'wb');
                            fwrite($filehandler3, base64_decode(trim($base64_string3)));
                            fclose($filehandler3);
                            $model->slip_doc3 = $newfile3;
                        }

                        if ($model->save(false)) {
                            $payment_receive_id = $model->id;
                        }
                    }

                    if ($payment_receive_id > 0) {
                        $order_id = $data_list[$i]['order_id'];
                        $pay_amount = $data_list[$i]['pay_amount'];

                        $model_line = new \common\models\PaymentReceiveLine();
                        $model_line->payment_receive_id = $payment_receive_id;
                        $model_line->order_id = $order_id;
                        $model_line->payment_amount = $pay_amount;
                        $model_line->payment_channel_id = $payment_channel_id;
