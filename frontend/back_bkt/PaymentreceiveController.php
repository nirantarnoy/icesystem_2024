...
//            ],
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $line_remain_amount = \Yii::$app->request->post('line_remain');
                ...
                if ($line_order != null) {
                    if (count($line_order) > 0) {
                        ...
                        for ($i = 0; $i <= count($line_order) - 1; $i++) {
                            $line_remain_original = str_replace(',', '', $line_remain_amount[$i]);
                            ...
                            if ($model_line->save(false)) {
                                if($model_line->payment_amount >= $line_remain_original){ //ถ้าชําระเต็ม
            ...
            if ($model) {
                ...
                if ($model_line) {
                    foreach ($model_line as $key => $value) {
                        $this->updatePaymenttransline($model->customer_id, $value->order_id, $value->payment_amount, 0);
                        if(\common\models\Orders::updateAll(['payment_status'=>0],['id'=>$value->order_id])){
                            \common\models\PaymentReceiveLine::deleteAll(['payment_receive_id' => $id]);
                        }
    ...
    public function actionGetitemnew()
    {
        ...
        if ($cus_id) {
           // $this->calPaymentTrans($cus_id);
            ...
            $sql .= " AND year(t1.order_date)>=2025";
...
//                    $html .= '<td style="text-align: center"><input type="file" class="form-control"></td>';
    ...
    public function calPaymentTrans($customer_id){
        if($customer_id){
            $findcaldate = date('Y-m-d', strtotime('-3 day'));
            $cal_date = date('Y-m-d');
            ...
            $sql = "select pl.order_id,sum(ol.line_total) as order_total,sum(pl.payment_amount) as payment_amount
                   from payment_receive as pr 
                   inner join payment_receive_line as prl on pr.id = prl.payment_receive_id
                   inner join order_line as ol on ol.id = prl.order_line_id
                   where pr.customer_id = $customer_id and date(pr.trans_date) >= '$findcaldate' and pr.payment_date <= '$cal_date' group by prl.order_id";
