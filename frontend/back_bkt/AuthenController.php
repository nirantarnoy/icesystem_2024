...
class AuthenController extends Controller
{
    ...
    public function actionLogin()
    {
        ...
        if ($username != '' && $password != '') {
            $model = \common\models\User::find()->where(['username' => $username])->andFilterWhere(['status' => 1])->one();
        //    $model = \common\models\User::find()->where(['username' => $username])->andFilterWhere(['status' => 0])->one();
