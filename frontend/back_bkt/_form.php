...
$model_doc = null;
$model_cus_req_id = \common\models\CustomerRequest::find()->where(['customer_ref_id' => $model->id])->one();
if($model_cus_req_id != null){
    $model_doc = \common\models\CustomerRequestDoc::find()->where(['customer_req_id' => $model_cus_req_id->id])->all();
}

if($model_doc == null){
    $model_doc = \common\models\CustomerRequestDoc::find()->where(['customer_id' => $model->id])->all();
}
...
<div class="customer-form">
    ...
    <div class="row">
        <div class="col-lg-3">
            <?= $form->field($model, 'use_province_id')->widget(\kartik\select2\Select2::className(), [
        ...
        <div class="col-lg-3">
            <?= $form->field($model, 'use_zone_id')->widget(\kartik\select2\Select2::className(), [
                'data' => \yii\helpers\ArrayHelper::map(\backend\helpers\Zone::asArrayObject(), 'id', 'name'),
                'options' => [
                    'placeholder' => '--โซน--'
                ]
            ]) ?>
    ...
    <div class="row">
        <div class="col-lg-3">
                ...
                <?= $form->field($model, 'sale_id')->Widget(\kartik\select2\Select2::className(), [
                    'data' => \yii\helpers\ArrayHelper::map(\backend\models\Employee::find()->where(['company_id' => $company_id, 'branch_id' => $branch_id,'status'=>1,'position'=>14])->all(), 'id', function ($data) {
...
//                    ?>
    ...
    <div class="row">
               ...
               <tr>
                ...
                <?php if($model_doc!=null):?>
                   <?php foreach($model_doc as $value):?>
                       <tr>
                           <td>
                               <a class="badge badge-pill badge-primary" href="<?=\Yii::$app->getUrlManager()->getBaseUrl() . '/uploads/files/customerrequest/'. $value->doc_name ?>" target="_blank"><?= $value->doc_name ?></a>
                           </td>
                       </tr>
                   <?php endforeach; ?>
                <?php else:?>
                    ...
                    <tr>
                        <td>
                            <input type="file" name="shopfile" multiple>
                        </td>
                    </tr>
                <?php endif;?>
                </tbody>
        ...
        <div class="col-lg-6">
            <div class="row">
                <div class="col-lg-3">
                    <?php echo $form->field($model, 'contact_no')->textInput(['maxlength' => true]) ?>
                </div>
                <div class="col-lg-6">
                    <div style="height: 30px"></div>
                    <?php if($model->contact_file != null):?>
                        <a class="badge badge-pill badge-info" href="<?= \Yii::$app->getUrlManager()->getBaseUrl() . '/uploads/files/customers/' . $model->contact_file?>" target="_blank"><?= $model->contact_file?></a>
                    <?php endif;?>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-6">
                    <input type="file" name="contact_file" value="">
                    <input type="hidden" name="old_contact_file" value="<?= $model->contact_file?>">
                </div>
            </div>
        </div>
