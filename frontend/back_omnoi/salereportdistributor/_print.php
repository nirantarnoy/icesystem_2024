...
$model_line = \common\models\Customer::find()->select(['id'])->where(['status'=>1,'is_show_pos'=>1])
...
function getOrderQty2($customer_id, $product_id, $from_date, $to_date)
{
    ...
    if ($customer_id && $product_id) {
        $model_qty = \common\models\QuerySaleByDistributor::find()->select(['SUM(qty) as qty'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d',strtotime($to_date))])->andFilterWhere(['customer_id' => $customer_id, 'product_id' => $product_id])->groupBy(['product_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->qty);
        }
    }
...
function getFree($customer_id, $from_date, $to_date)
{
    $data = 0;
    if ($customer_id) {
        $model_qty = \common\models\QuerySaleByDistributor::find()->select(['SUM(qty) as free_qty'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['customer_id' => $customer_id])->andFilterWhere(['price'=>0])->groupBy(['customer_id'])->one();
        if ($model_qty != null) {
...
function getCashAmount($customer_id, $product_id, $from_date, $to_date)
{
    $data = 0;
    if ($customer_id && $product_id) {
        $model_qty = \common\models\QuerySaleByDistributor::find()->select(['SUM(line_total) as line_total'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['customer_id' => $customer_id, 'product_id' => $product_id,'payment_method_id'=>1])->groupBy(['product_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->line_total);
        }
    }
    return $data;
}
...
function getCreditAmount($customer_id, $product_id, $from_date, $to_date)
{
    ...
    if ($customer_id && $product_id) {
        $model_qty = \common\models\QuerySaleByDistributor::find()->select(['SUM(line_total) as line_total'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['customer_id' => $customer_id, 'product_id' => $product_id,'payment_method_id'=>2])->groupBy(['product_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->line_total);
        }
    }
...
//        }
        $model_qty = \common\models\QueryPaymentReceive::find()
            ->where(['BETWEEN', 'date(trans_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['customer_id' => $customer_id,'payment_method_id'=>1])->sum('payment_amount');
        if ($model_qty != null) {
            $data = ($model_qty);
        }
    }
